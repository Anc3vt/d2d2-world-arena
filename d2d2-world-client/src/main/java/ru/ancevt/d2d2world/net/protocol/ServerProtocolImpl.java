...
public final class ServerProtocolImpl extends ProtocolImpl {
    ...
    public void bytesReceived(int playerId, byte[] bytes) {
        ...
        switch (message.getType()) {
            case MessageType.CLIENT_PLAYER_ENTER_REQUEST -> {
                ...
                serverProtocolImplListeners.forEach(l ->
                        l.playerEnterRequest(playerId, name, clientProtocolVersion, extraData));
            }

            case MessageType.CLIENT_PLAYER_EXIT_REQUEST -> {
                log.debug("received CLIENT_PLAYER_EXIT_REQUEST");
                serverProtocolImplListeners.forEach(l -> l.playerExitRequest(playerId));
            }
            ...
            case MessageType.CLIENT_PLAYER_CONTROLLER_AND_XY_REPORT -> {
                ...
                serverProtocolImplListeners.forEach(l -> l.playerControllerAndXYReport(playerId, controlState, x, y));
            }
            ...
            case MessageType.CLIENT_PLAYER_TEXT_TO_CHAT -> {
                ...
                serverProtocolImplListeners.forEach(l -> l.playerTextToChat(playerId, text));
            }
            ...
            case MessageType.CLIENT_RCON_LOGIN -> {
                ...
                serverProtocolImplListeners.forEach(l -> l.rconLogin(playerId, login, passwordHash));
            }
            ...
            case MessageType.CLIENT_RCON_COMMAND -> {
                ...
                serverProtocolImplListeners.forEach(l -> l.rconCommand(playerId, commandText, extraData));
            }

            case MessageType.CLIENT_PLAYER_PING_REQUEST -> {
                log.debug("received CLIENT_PLAYER_PING_REQUEST");
                serverProtocolImplListeners.forEach(l -> l.playerPingRequest(playerId));
            }

            case MessageType.CLIENT_PLAYER_PING_REPORT -> {
                log.debug("received CLIENT_PLAYER_PING_REPORT");
                int ping = in.readShort();
                serverProtocolImplListeners.forEach(l -> l.playerPingReport(playerId, ping));
            }

            case MessageType.EXTRA -> {
                log.debug("received EXTRA");
                String extraDataFromPlayer = in.readUtf(int.class);
                serverProtocolImplListeners.forEach(l -> l.extraFromPlayer(playerId, extraDataFromPlayer));
            }
